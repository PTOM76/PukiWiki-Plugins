<?php
// $Id: ctrlcmt.inc.php,v 1.02 2023/10/01 16:57:00 Pitan Exp $
...
define('PLUGIN_CTRLCMT_NONAME', "名無しさん"); // 空欄時の名前
//----
...
define("PLUGIN_CTRLCMT_IPCRYPASS", "abcdef"); // 2ch風IDのパスワード (変更は任意)

function plugin_ctrlcmt_init() {
    // Language
	$messages = array(
		'_ctrlcmt_messages' => [
            'ja' => [
                ...
                'page' => 'ページ',
                'ip_address' => 'IPアドレス',
                "err_iplog" => 'ログは存在しません。<br />この機能を利用したい場合は「コメント/IPアドレスの保存」を有効にして下さい。',
            ...
            'en' => [
                ...
                'err_iplog' => 'Log does not exist. <br />If you want to use this feature, please enable "Save comments/IP address".',
        ...
        if (pkwk_login($_SESSION['ctrlcmt_loginpass'])){
            ...
            if (file_exists(PLUGIN_CTRLCMT_DIR . '/cmt/log.json'))
            {
                ...
                foreach($log_json as $value){
                    $commentlogs .= <<<EOD
                    <tr>
                        <td class="style_td">{$lang['name']}</td>
                        <td class="style_td">{$value['name']}</td>
                    </tr>
                    <tr>
                        <td class="style_td">{$lang['comment']}</td>
                    ...
                    <tr>
                        <td class="style_td">{$lang['date']}</td>
                        <td class="style_td">{$value['date']}</td>
                    </tr>
                    <tr>
                        <td class="style_td">{$lang['page']}</td>
                        <td class="style_td">{$value['page']}</td>
                    </tr>
                    <tr>
                        <td class="style_td">{$lang['ip_address']}</td>
                ...
                $commentlogs = <<<EOD
                <tr>
                    <td class="style_td" style="text-align:center;" colspan="2">{$lang['err_iplog']}</td>
                </tr>
                EOD;
            }
            //-
            ...
            <h2>{$lang['controlpanel']}</h2>
            <span id="form_result"></span>
            <ul class="form_tab">
                <li class="form_active">{$lang['settings']}</li>
                <li>{$lang['comment_log']}</li>
                {$addons_tab}
                <li>{$lang['logout']}</li>
            </ul>
